// cargo run --release --package ssle_core --example check_commit2 -- -t 1 -p 4
// cargo +nightly run --release --package ssle_core --example check_commit2 --features="nightly" -- -t 1 -p 4
...
fn party_operation(
    ...
    let mut check_commit: RlweOwned<CommitValueT> = Rlwe::zero(commit_rlwe_len);

    for (i, (commit, commit_pk, rr_commit)) in izip!(
        all_commit.iter(),
        all_commit_pk.iter(),
    ...
    .enumerate()
    {
        ...
        if i == 0 {
            check_commit.as_mut().copy_from_slice(rr_commit.as_ref());
        }
    }

    let mut all_check_commit: Vec<RlweOwned<CommitValueT>> =
        vec![Rlwe::zero(commit_rlwe_len); party_count];
    ...
    if party_id == 0 {
        ...
        let mut check_commit = check_commit.clone().into_ntt_form(&commit_ntt_table);
        check_commit
            .iter_ntt_poly_mut(poly_length)
            .for_each(|poly| {
                NttPolynomial(ArrayBase(poly))
                    .mul_assign(&NttPolynomial(ArrayBase(temp.as_ref())), CommitModulus)
            });

        println!("expired partial commit = {:?}", check_commit.as_ref());
        println!("real partial commit = {:?}", temp_commit.as_slice());
    }

    party.share_v3(&check_commit, all_check_commit.as_mut_slice());
    check_commit.set_zero();
    for commit in all_check_commit.iter() {
        check_commit.add_element_wise_assign(commit, CommitModulus);
    }

    if party_id == 0 {
        check_commit.iter_poly_mut(poly_length).for_each(|poly| {
            commit_ntt_table.transform_slice(poly);
        });
        let cipher = NttRlwe::new(ArrayBase(check_commit.as_ref()));

        let m = commit_sk.decrypt(&cipher, commit_params, &commit_ntt_table);
        assert!(m.is_zero());
        println!("expired commit = {:?}", check_commit.as_ref());
    }
    ...
    if let Ok(mut final_commit) = recv_final_commit.recv() {
        ...
        if party_id == 0 {
            println!("real commit = {:?}", final_commit.as_slice());
        }
